...
const AdminPanel = () => {
  ...
  useEffect(() => {
    ...
    if (user.role !== 'admin') {
      ...
      try {
        ...
        const response = await axios.get('https://rushikersclub-backend.azurewebsites.net/api/v1/magazine/pending', {
          headers: {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      await axios.put(`https://rushikersclub-backend.azurewebsites.net/api/v1/magazine/approve/${id}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`, // Include the token in the request headers
        },
        withCredentials: true,
